...
check_filesystem() {
    ...
    if ! fs_info=$(df -k "$dir" 2>/dev/null | tail -n 1); then
        log_message "$dir 의 파일시스템 정보를 읽지 못했습니다" "error"
        return 1
    fi
    ...
    if [ "$available" -lt "$required_space" ]; then
        log_message "디스크 여유 공간 경고: 1GB 미만 남았습니다" "warning"
    fi
...
cleanup() {
    ...
    log_deletion() {
        ...
        if rm -f "$file"; then
            ...
            log_message "삭제에 실패했습니다: $file" "error"
        fi
    }
    ...
    if ((deleted_count > 0)); then
        log_message "$deleted_count 개 파일을 정리해 $(format_size "$total_size") 를 확보했습니다 (${duration}초)" "success"
        local category
